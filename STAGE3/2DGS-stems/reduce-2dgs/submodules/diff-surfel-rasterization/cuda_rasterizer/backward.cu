...
__global__ void preprocessCUDA(
	...
	glm::vec2* dL_dscales,
	glm::vec4* dL_drots,
	float* dL_dopacity, /*  1月19日 修改点3.2增加两个参数  */
	const float sh_sparsity_multiplier)
{
	...
	if (shs)
		computeColorFromSH(idx, D, M, (glm::vec3*)means3D, *campos, shs, clamped, (glm::vec3*)dL_dcolors, (glm::vec3*)dL_dmean3Ds, (glm::vec3*)dL_dshs, sh_sparsity_multiplier);/* 1月19日 修改点3.2.1 颜色计算 */
	...
	// dL_dopacity[idx] *= conic_opacities[idx].w * (1.0 - conic_opacities[idx].w); /* 1月19日 修改点3.2.2 不透明度梯度 */
}
...
{	
	preprocessCUDA<NUM_CHANNELS><< <(P + 255) / 256, 256 >> > (
		...
		dL_dscales,
		dL_drots,
		dL_dopacity, /* 1月19日 修改点3.3 合理调用添加两个参数 */
