...
/* 1月16日 修改点2.1 加入从可变SH得到Color的优化方案 */
...
/* 1月16日 修改点2.2 加入可变SH优化方案 *****************************************************************************************************/
// Perform initial steps for each Gaussian prior to rasterization.
template<int C>
__global__ void variableSHPreprocessCUDA(int P, int *D, int M, const int* coeffsNum,  /* 1月16日修改点2.3 为了可变SH加入三个相应的参数: */
    const int* perBandPrimitiveCount,
	const int* cumSumPrimitiveCount,
	const float* orig_points,
	const glm::vec2* scales,
	...
	bool prefiltered)
{
	...
	// Compute colors 
	if (colors_precomp == nullptr) {
		glm::vec3 result = computeColorFromSH(idx, coeffsNum, perBandPrimitiveCount, cumSumPrimitiveCount, (glm::vec3 *)orig_points, *cam_pos, shs, clamped); // 1月16日 修改点2.4修改颜色计算方法
		rgb[idx * C + 0] = result.x;
		rgb[idx * C + 1] = result.y;
		rgb[idx * C + 2] = result.z;
	}
...
renderCUDA(
	...
	int* __restrict__ out_touched_pixels,     /* 1月14日  修改点1.1--新添3个变量 267-269 */
	...
	const bool calculate_mean_transmittance)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
            ...
            /* 1月14日 修改点1.2--新增touched_pixels与transmittance的计算  */
...
void FORWARD::render(
	...
	float* out_color,
	float* out_others,
	int* out_touched_pixels, /* 1月14日修改点1.3  新增三个参数*/
	...
	const bool calculate_mean_transmittance)
{
	renderCUDA<NUM_CHANNELS> << <grid, block >> > (
		...
		out_color,
		out_others,
		out_touched_pixels,  /* 1月14日修改点1.4  新增三个参数*/
		out_transmittance,
...
/* 1月16日 修改点2.5 加入推理的预处理操作 */
void FORWARD::inferencePreprocess(int P, int *D, int M,const int bandsNum, const int* coeffsNum, 
    	const int* perBandPrimitiveCount,
		const int* cumSumPrimitiveCount,
		const float* means3D,
		const glm::vec2* scales,
		...
		bool prefiltered)
{
	variableSHPreprocessCUDA<NUM_CHANNELS> << <(P + 255) / 256, 256 >> > (
		P, D, M,coeffsNum, perBandPrimitiveCount,
