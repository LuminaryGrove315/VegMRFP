...
RasterizeGaussiansVariableSHBandsCUDA(
	...
	const torch::Tensor &perBandPrimitiveCount,/* 1月19日 修改点2.10.1 添加相应的三个变量 *****************/
	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::inferenceForward(
		...
		binningFunc,
		imgFunc,
		P, degrees.contiguous().data<int>(), M,bandsNum, coeffsNum.contiguous().data<int>(), /* 1月19日 修改点2.10.2 添加相应的三个参数 */
	...
	const bool debug)
{
  ...
  std::function<char*(size_t)> binningFunc = resizeFunctional(binningBuffer);
  std::function<char*(size_t)> imgFunc = resizeFunctional(imgBuffer);
  //torch::Tensor out_touched_pixels = torch::empty({0}, int_opts);/* 1月14日 修改点1.9 声明两个变量用来存储相应的值 */ 
  //torch::Tensor out_transmittance = torch::empty({0}, float_opts);
  torch::Tensor out_touched_pixels = torch::full({P, 1}, 0, int_opts);
  torch::Tensor out_transmittance = torch::full({P, 1}, 0.f, float_opts);
  int rendered = 0;
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_color.contiguous().data<float>(),
		out_others.contiguous().data<float>(),
		out_touched_pixels.contiguous().data<int>(),/* 1月14日 修改点1.10 为了完美调用forward函数,参数需要进行相应修改 */
		out_transmittance.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		true, 
		debug);
